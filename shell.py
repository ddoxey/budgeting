...
class Data:
    ...
    def read_exception(categories):
        ...
        threshold = 15
        records = sorted([add_epoch(record) for record in exceptions],
...
class BudgetShell(cmd.Cmd):
    ...
    def do_add(self, arg_str):
        ...
        add_exception_regex = re.compile((
            r'\A exception \s+ '
            r'   (\w+) \s+ '
            r'   (\d{2}[-]\d{2}[-]\d{4}) \s+ '
            r'   ([-]? [$]? \d+ (?: [.]\d+ )?) '
            r'\Z'), re.X | re.M | re.S | re.I)
        add_transaction_regex = re.compile((
            r'\A transaction \s+ '
            r'   (\w+) \s+ '
            r'   (\S+) \s+ '
            r'   ([-]? [$]? \d+ (?: [.]\d+ )?) '
            r'\Z'), re.X | re.M | re.S | re.I)
        m = add_theme_regex.match(arg_str)
        if m is not None:
            cat = m.group(1)
            ...
            self.themes_changed = True
            return
        m = add_exception_regex.match(arg_str)
        if m is not None:
            cat = m.group(1)
            if cat not in self.categories:
                print(f'Unrecognized category: {cat}', file=sys.stderr)
            ...
            self.exceptions_changed = True
            return
        m = add_transaction_regex.match(arg_str)
        if m is not None:
            cat = m.group(1)
        ...
        print(f'Invalid add statement: {arg_str}', file=sys.stderr)
        return

    def complete_add(self, text, state, begidx, endidx):
        add_types = ['transaction', 'exception', 'theme']
        tokens = re.split(r'\s+', state.strip())
        if len(tokens) == 1:
            return add_types
        add_type = tokens[1]
        if len(tokens) == 2:
            if add_type not in add_types:
                return [opt for opt in add_types
                        if opt.startswith(add_type)]
        ...
        if len(tokens) == 3:
            ...
            if add_type in ['theme', 'exception']:
                return self.get_predicted_dates(cat)
        if len(tokens) == 4:
            cat, date = tokens[2:4]
            if add_type == 'exception':
